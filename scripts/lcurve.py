...
parser.add_argument(
    "-path", dest="path", help="Path to the observation files", nargs="?", type=str
)
parser.add_argument("-name", dest="name", help="Source name", nargs="?", type=str)
parser.add_argument(
    "-obs", help="Observation identifier", nargs="?", type=str, default=""
)
...
parser.add_argument(
    "-gti", help="Path to the GTI of the observation", nargs="?", type=str, default=None
)
...
parser.add_argument(
    "-mode",
    dest="mode",
    help="Plot style: monochrome / medium / color",
    nargs="?",
    type=str,
    default="medium",
)
...
if args.src == None:
    lccorr = "{0}/{1}/lcurve_{2}_{4}_{5}/{3}_lccorr_{2}.lc".format(
        args.path, args.obs, args.ft, args.name, args.inst, args.n
    )
    print(args.name)
    if path.exists(lccorr):
        args.src = lccorr
    else:
        args.src = "{0}/{1}/lcurve_{2}_{4}_{5}/{3}_lc_{2}_src.lc".format(
            args.path, args.obs, args.tw, args.name, args.inst, args.n
        )
        args.bgd = "{0}/{1}/lcurve_{2}_{4}_{5}/{3}_lc_{2}_bgd.lc".format(
            args.path, args.obs, args.tw, args.name, args.inst, args.n
        )
...
# else:
out = "{0}/{1}/lcurve_{2}_{4}_{5}/{3}_lc_{2}.pdf".format(
    args.path, args.obs, args.tw, args.name, args.inst, args.n
)
...
if "mono" in args.mode:
    # Data
    plt.errorbar(
        time, cts, yerr=std, fmt="o", color="k", markersize=4, elinewidth=1.0, zorder=6
    )
...
plt.text(
    0.1,
    1.03,
    r"P($\chi^2$) = {0:.2e} ".format(args.pcs),
