...
def find_simbad(my_ra, my_dec):
	...
	try:
		...
		simbad_result_table = custom_simbad.query_region(coord.SkyCoord(ra=my_ra, dec=my_dec,unit=(u.deg, u.deg), frame='fk5'),radius=0.83 * u.deg)
		...
		for i in range(loop_length):
			...
			print ("RA=", RA, "DEC=", DEC, "SEP=", SEP_ARCSEC, " obj_name= ",OBJ_NAME, "obj_type=",OBJ_TYPE)
			if(SEP_ARCSEC < 30.01):
				if(i==0):
	...
	return match1_sep, match1_obj_name, match1_obj_type, match2_sep, match2_obj_name, match2_obj_type, match3_sep, match3_obj_name, match3_obj_type, match4_sep, match4_obj_name, match4_obj_type, match5_sep, match5_obj_name, match5_obj_type
...
with open('triple_match_non_bright.csv','r') as csvinput:
	with open('triple_match_obs_sim.csv', 'w') as csvoutput:
		...
		row.append('match4_sep')
		row.append('match4_obj_name')
		row.append('match4_obj_type')
		row.append('match5_sep')
		row.append('match5_obj_name')
		row.append('match5_obj_type')
		all.append(row)

		for row in reader:
			ra= row[col_no_ra]
			dec= row[col_no_dec]
			match1_sep, match1_obj_name, match1_obj_type, match2_sep, match2_obj_name, match2_obj_type, match3_sep, match3_obj_name, match3_obj_type, match4_sep, match4_obj_name, match4_obj_type, match5_sep, match5_obj_name, match5_obj_type = find_simbad(ra,dec)
			print(match1_sep)
			row.append(match1_sep)
			row.append(match1_obj_name)
			row.append(match1_obj_type)
