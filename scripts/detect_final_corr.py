...
def check_triple(corr_1, corr_2, corr_3):
    ...
    for cand in src_cand:
        t1 = corr_3[
            np.where((corr_3["ID_1"] == cand[0]) & (corr_3["INST_1"] == cand[1]))
        ]
        t2 = corr_2[
            np.where((corr_2["ID_1"] == cand[2]) & (corr_2["INST_1"] == cand[3]))
        ]

        for line1 in t1:
            for line2 in t2:
                if (
                    line1["ID_2"] == line2["ID_2"]
                    and line1["INST_2"] == line2["INST_2"]
                ):
                    triple.append(
                        [
                            line1["ID_1"],
...
corr_table = Table(
    names=("ID_1", "INST_1", "RA_1", "DEC_1", "ID_2", "INST_2", "RA_2", "DEC_2", "SEP"),
    dtype=("i2", "U25", "f8", "f8", "i2", "U25", "f8", "f8", "f8"),
)
...
corr_table = check_correlation(
    src_PN, src_M1, corr_table, sep_cutoff=seperation_cut_off
)
corr_table = check_correlation(
    src_M1, src_M2, corr_table, sep_cutoff=seperation_cut_off
)
corr_table = check_correlation(
    src_PN, src_M2, corr_table, sep_cutoff=seperation_cut_off
)

# Sorting the table
corr_PN_M1 = corr_table[
    np.where((corr_table["INST_1"] == "PN") & (corr_table["INST_2"] == "M1"))
]
corr_M1_M2 = corr_table[
    np.where((corr_table["INST_1"] == "M1") & (corr_table["INST_2"] == "M2"))
]
corr_PN_M2 = corr_table[
    np.where((corr_table["INST_1"] == "PN") & (corr_table["INST_2"] == "M2"))
]
...
if os.path.exists("./triple_match.csv"):
    ...
    master_file.write(
        "OBS_ID,class,PN_REGION_NUMBER,PN_RA,PN_DEC,M1_REGION_NUMBER,M1_RA,M1_DEC,M2_REGION_NUMBER,M2_RA, M2_DEC,PN_M1_SEP,PN_M2_SEP, M1_M2_SEP\n"
    )
...
if include_singles:
    ...
    for ii in range(len(src_PN)):
        ...
        if present == 0:
            line = """XMM {0},S,{1},{2},{3},-,-,-,-,-,-,-,-,-\n""".format(
                str(args.obs), src_PN[ii]["ID"], src_PN[ii]["RA"], src_PN[ii]["DEC"]
            )
            master_file.write(line)
    ...
    for ii in range(len(src_M1)):
        ...
        if present == 0:
            line = """XMM {0},S,-,-,-,{1},{2},{3},-,-,-,-,-,-\n""".format(
                str(args.obs), src_M1[ii]["ID"], src_M1[ii]["RA"], src_M1[ii]["DEC"]
            )
            master_file.write(line)
    ...
    for ii in range(len(src_M2)):
        ...
        if present == 0:
            line = """XMM {0},S,-,-,-,-,-,-,{1},{2},{3},-,-,-\n""".format(
                str(args.obs), src_M2[ii]["ID"], src_M2[ii]["RA"], src_M2[ii]["DEC"]
            )
            master_file.write(line)
